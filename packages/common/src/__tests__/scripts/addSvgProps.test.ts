...
describe("addSvgProps", () => {
  ...
  it("should update the SVG props correctly for mobile icons", async () => {
    ...
    expect(icon1Data).toContain(
      // prettier-ignore
      "const SvgIcon1 = ({ width = 24, height = 24, fill = \"none\", stroke = \"#000\", strokeWidth = 2, ...props }: SvgProps) => ("
    )
    ...
    expect(icon2Data).toContain(
      // prettier-ignore
      "const SvgIcon2 = ({ width = 32, height = 32, fill = \"red\", stroke = \"#111\", strokeWidth = 3, ...props }: SVGProps<SVGSVGElement>) => ("
    )
  ...
  it("should update the SVG props correctly for web icons", async () => {
    ...
    expect(icon3Data).toContain(
      // prettier-ignore
      "const SvgIcon3 = ({ width = 16, height = 16, fill = \"blue\", stroke = \"#222\", strokeWidth = 1.5, ...props }: SVGProps<SVGSVGElement>) => ("
    )
