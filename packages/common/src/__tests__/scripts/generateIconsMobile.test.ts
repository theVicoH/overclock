...
describe("runScript", () => {
  it("should execute the correct command", () => {
    ...
    expect(execSync).toHaveBeenCalledWith(`cross-env svgr --typescript --native --out-dir ${outDir} ${srcDir}`, { stdio: "inherit" })
  })

  it("should resolve correct paths", () => {
    expect(outDir).toBe(path.resolve(__dirname, "../../icons/mobile"))
